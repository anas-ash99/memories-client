...
import { addFriend } from '../../../redux/usersReducer'
...
export default function NotfyContainer({notification}) {
  ...
  //  }
   ...
   const addFriends = () => {
    ...
    Axios.put(`/users/addFriend/${loggedInUser._id}/${notification.action_userId}`).then(res=>{
        if (res.data === "added") {
            // dispatch(userSrearching(false))
            dispatch(addFriend(notification.action_userId))
            alert(`${notification.content.userName } added`)
